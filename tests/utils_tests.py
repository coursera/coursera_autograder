...
def test_chattiness_parsing_quiet():
    ...
    args = parser.parse_args('-qq configure'.split())
    print('args: ', args)
    assert args.quiet == 2
...
def test_chattiness_parsing_verbose():
    parser = main.build_parser()
    args = parser.parse_args('-v configure'.split())
    assert args.verbose == 1


def test_set_logging_level():
    parser = main.build_parser()
    args = parser.parse_args('-vv configure'.split())
...
def test_set_timeout():
    parser = main.build_parser()
    args = parser.parse_args('--timeout 120 configure'.split())
